...
- 本记录生成前 HEAD：`331daed9fd39557d15bea4b5ab4ba8e28e39d9bc`；文档提交会改变 HEAD，当前值请用 `git rev-parse HEAD` 校验。
...
```text
pytest tests/class_assistant -q  -> 127 passed
...
阶段一至五提交：`4f60227`（Fork/基线记录）、`376c30d`（前端锁定）、`89b929e` 至 `b4a806b`（WCDB preflight、只读群元数据发现、生命周期及日志脱敏加固）、`180ae3e`（Checkpoint 5 离线验收覆盖），以及后续安全修复提交（复合游标、采集租约、回调确认、发送闸门和生命周期清理）。本次文档修订提交以 `git log -1 --oneline` 为准。Fork 已绑定到 `https://github.com/zbw2007/webot.git`；前端使用 npm 官方源完成安装并生成 `ui/package-lock.json`；`npm audit --omit=dev` 报告 0 vulnerabilities，`npm run build` 已通过（仅有 Vite CommonJS 配置和 chunk 体积提示）。
...
- 白名单成员读取已与消息采集隔离：群发现只返回群 ID、名称和人数，不返回成员 wxid、消息正文或私聊数据；采集器继续只接受显式白名单群。
- WCDB 轮询使用 `(timestamp, message_id)` 复合游标，按页读取并在数据库写入成功后推进；游标可跨实例持久化，旧游标不会被较新的进程回退覆盖。
- 回调交付增加连续确认和 inflight 台账：回调失败、停止或游标持久化失败时不会提交游标；仅连续完成的消息才能推进游标，避免跳过同一时间戳的消息。
- SQLite 增加 WCDB 采集租约，按后端实例和群竞争、续租、释放；启动失败、停止和轮询退出都会清理租约，避免多实例重复读取。
- 发送闸门在发送前再次校验批准版本、一次性确认令牌、白名单群、窗口标题和重复指纹；关闭或停止期间禁止新发送，发送中的任务完成后才释放资源。
- 启动、重启失败和停止路径均清理 WCDB client、群映射、回调任务和租约；发送崩溃保持 `sending`，必须人工对账，禁止自动重发。
...
- 本次离线验收：专项测试覆盖白名单采集、群成员读取隔离、双群独立分析、复合游标分页与跨实例持久化、回调连续确认与 inflight、防重复采集、SQLite 采集租约、来源关联、编辑撤销批准、一次性确认令牌（含复用拦截）、DRY_RUN 模拟发送、并发 claim、发送闸门、启动/停止清理、崩溃对账和模型失败回滚；`pytest tests/class_assistant -q` 实际结果为 `127 passed`。`python -m compileall -q src tests`、`pip check`、`git diff --check` 和前端 `npm run build` 均已通过。
- 当前保持 `CLASS_ASSISTANT_ENABLED=false`、`REAL_SEND_ENABLED=false`、`DRY_RUN=true`，未连接主微信、未读取真实群消息、未操作发送器。
- Fork 和前端构建已完成；可信 WCDB DLL 来源、签名、批准哈希和微信版本兼容性仍未确认，测试账号/测试群尚未配置，阶段六至八的 48 小时只读、分析和模拟发送尚未完成。因此不能启动真实采集或进入真实账号上线阶段。
